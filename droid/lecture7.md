...
- **R** class gives us access to all the resources in our project
    ...
    - access through the R class, it returns a resource *id* as an integer
    ```kotlin
    R.drawable.krabs
    ```
...
- When Jetpack Compose executes a composable, it enter the *Composition*
- Two ways to enter the *Composition*
    1. The firs time you run your composable it goes through *intial composition*
...
## State
- To create *state* we wrap a value in a **State object**
    - **mutableStateOf** returns an interface
        ...
        - Any composable functinos that reavd *value* observe and schedule a recomposition when that value changes
        ```kotlin
        Text(text = "${num.value}")
        ```
        ...
        > The Text composable will go through *recomposition* when the Button is clicked
        > It's not the button composbale, but intelligent recomposition
- Intelligent Recomposition
    - recompose onlhy the components that *read value*
    - others; ignore
...
- State should be **internal**(local) to the composable, with a few exceptions
    ...
    - if we move our state object inside the composable you might get an error
        ...
        fun counter() {...}
        ```
    - Without *remember*, the Text composable will still go through recomposition when the button is clicked
...
- **mutableStateListOf**
    ...
    ```
    - The *mutableStateListOf* function uses a different mechanims to trigger a recomposition
        - Snapshotting
    - Since it doesn't use the *MutableState<T>* interface, we don't use the value property
        - we also wouldn't use the *by* keyword
...
- you can make any composable clickable
    - clickable modifier provides an *onClick* event callback
